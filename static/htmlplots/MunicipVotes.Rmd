...
rows <- tribble(~term, ~`Spatial Error`,  ~`Spatial Lag`, ~`Error + Lag`, ~`Spatial Error`,  ~`Spatial Lag`, ~`Error + Lag`,
                'Num.Obs',        "1761", "1761", "1761", "1761", "1761", "1761")
attr(rows, 'position') <- c(21)


modelsummary(list("Spatial Error" = sp_err_turnout_2021, 
                  "Spatial Lag" = sp_lag_turnout_2021,
                  "Error + Lag" = sp_sacsar_turnout_2021,
                  "Spatial Error" = sp_err_turnout_rel_2021, 
                  "Spatial Lag" = sp_lag_turnout_rel_2021,
                  "Error + Lag" = sp_sacsar_turnout_rel_2021),
             stars = TRUE,
             add_rows = rows,
             coef_rename = c("Fstrength" = "Fiscal strength", 
                             "EldPop" = "Elderly",
...
models_V <- list()
models_V[['Fixed effects lag']] <- mod_grp_V
models_V[['Fixed effects error']] <- mod_grp_V1
models_V[['Fixed effects SARAR']] <- mod_grp_V2

mod1 <- modelsummary(models_V, add_rows = ar, stars = T, 
             output = "kableExtra",
             fmt = 3)
...
ar <- as.data.frame(rbind(c("R2", (round(sp_lag_turnout_rel_sum$rsqr,2)), 
...
mod2 <- modelsummary(models_V, add_rows = ar, stars = T, 
             output = "kableExtra",
             fmt = 3)
...
ar <- as.data.frame(rbind(c("R2", (round(sp_lag_LDP_rel_sum$rsqr,2)), 
