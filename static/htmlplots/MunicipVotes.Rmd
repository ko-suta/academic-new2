...
Elections <- Elections %>%
    mutate(
    Kaso.fusion = case_when(
      Kaso %in% "Normal" & Fusion == 1   ~ "Normal-Merged",
      Kaso %in% "Normal" & Fusion == 0   ~ "Normal",
      Kaso %in% "一部過疎"               ~ "一部過疎",
      Kaso %in% "みなし過疎"               ~ "みなし過疎",
      Kaso %in% "全部過疎" & Fusion == 1 ~ "全部過疎-Merged",
      Kaso %in% "全部過疎" & Fusion == 0 ~ "全部過疎"
    )
  )
```
...
```{r include=FALSE}
# merged municipalities that "switched" from partly depopulating 
# to fully depopulating municipalities between 2009/2012 and 2021

Set1 <- Elections3 %>%
  filter(Year == "2009" &
           Fusion == 1 & 
           Kaso == "一部過疎") %>%
  select(Code, Pref, Municip, LDP, LDP_relative, Turnout_relative)
...
Set12 <- inner_join(Set1,Set2, by="Code")

Set12 <- Set12 %>%
  mutate(LDPdif = LDP.y - LDP.x,
         LDPdif_rel = LDP_relative.y - LDP_relative.x,
         Turnout_dif = Turnout_relative.y - Turnout_relative.x)

Set12 %>%
  summarise(mean(LDPdif, na.rm = TRUE),
            mean(LDPdif_rel, na.rm = TRUE),
            mean(Turnout_dif, na.rm = TRUE))
```
...
```{r echo=FALSE, results='asis', message=FALSE, warning=FALSE}
Elections3 %>%
  filter(Kaso.fusion %in% c("Normal", "Normal-Merged", "みなし過疎", 
  ...
  mutate(Kaso.fusion = factor(Kaso.fusion, levels = c("Normal",
                                                      "Normal-Merged",
                                                           ...
                                                           "全部過疎",
                                                           "全部過疎-Merged"), 
                      labels = c("Normal", "Normal-Merged", "Quasi-depopulating", 
                                 "Partly depopulating",
                                 "Fully depopulating",
...
```{r echo=FALSE, results='asis'}
Elections3[,-c(6,11,13:15,18:20,21:30)] %>%
  select_if(is.numeric) %>%
...
turnout.pooled <- plm(Turnout ~ Fstrength + LDP + Kaso.fusion + EldPop,
                  index=c("Code","Year"), data = Elections4, model = "within", effect = "time")

turnout.fe <- plm(Turnout ~ Year + Fstrength + LDP + Kaso.fusion + EldPop,
                  index=c("Code","Year"), data = Elections4, model = "within")

turnout.re <- plm(Turnout ~ Fstrength + LDP + Kaso.fusion + EldPop,
                  index=c("Code","Year"), data = Elections4, 
                  model = "random")
```
...
htmlreg(list(TurnoutMod2, TurnoutMod3,
  ...
  groups = list("Year dummy (base group = '2012')" = 2:4,
                "Fiscal type (base group = 'Normal')" = 9:13),
       caption = NULL,
       digits = 2,
...
LDPMod2 <- plm(LDP ~ Year + Fstrength + EldPop + Turnout + LATperCapita, 
               index=c("Code","Year"), model = "pooling", data = Elections4)
...
htmlreg(list(LDPMod2, LDPMod3,
       ...
       custom.coef.names = c("(Intercept)",
                             ...
                             "Partly depopulating", "Fully depopulating",
                             "Fully depopulating (merged)"),
       custom.header = list("LDP" = 1:2, "LDP (relative)" = 3:4),
       groups = list("Year dummy (base group = '2012')" = 2:4,
                "Fiscal type (base group = 'Normal')" = 9:13),
       caption = NULL,
       digits = 2,
...
htmlreg(list(LDPDifMod2, LDPDifMod3, LDPDifMod5, LDPDifMod6),
       ...
       custom.coef.names = c("(Intercept)",
                             ...
                             "Partly depopulating", "Fully depopulating",
                             "Fully depopulating (merged)"),
       custom.header = list("LDP: SMD-PR Dif." = 1:2, 
                            "LDP: SMD-PR Dif. (relative)" = 3:4),
       groups = list("Year dummy (base group = '2012')" = 2:4,
                "Fiscal type (base group = 'Normal')" = 9:13),
       caption = NULL,
       digits = 2,
...
d3 <- sf::read_sf("GIS/japan_ver81.shp") %>%
...
addnbs <- function(sp.sample){
    ...
    coords <- st_centroid(st_geometry(sp.sample))
...
<!--        "Kaso.fusionNormal-Merged" = "Normal (merged)", -->
...
<!--   pack_rows("Fiscal type (base group = ‘Normal’)", 11, 19) %>% -->
...
<!-- modelsummary(list("Spatial Lag" = sp_lag_LDP_2021, -->
<!--                   "Spatial Error" = sp_err_LDP_2021,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_2021, -->
<!--                   "Spatial Lag" = sp_lag_LDP_rel_2021, -->
<!--                   "Spatial Error" = sp_err_LDP_rel_2021,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_rel_2021), -->
<!--              stars = TRUE, -->
<!--              add_rows = rows, -->
<!--              coef_map = cm -->
<!--              ) %>% -->
...
<!-- modelsummary(list("Spatial Lag" = sp_lag_LDP_2017, -->
<!--                   "Spatial Error" = sp_err_LDP_2017,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_2017, -->
<!--                   "Spatial Lag" = sp_lag_LDP_rel_2017, -->
<!--                   "Spatial Error" = sp_err_LDP_rel_2017,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_rel_2017), -->
<!--              stars = TRUE, -->
<!--              add_rows = rows, -->
<!--              coef_map = cm -->
<!--              ) %>% -->
...
<!-- modelsummary(list("Spatial Lag" = sp_lag_LDP_2014, -->
<!--                   "Spatial Error" = sp_err_LDP_2014,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_2014, -->
<!--                   "Spatial Lag" = sp_lag_LDP_rel_2014, -->
<!--                   "Spatial Error" = sp_err_LDP_rel_2014,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_rel_2014), -->
<!--              stars = TRUE, -->
<!--              add_rows = rows, -->
<!--              coef_map = cm -->
<!--              ) %>% -->
...
<!-- modelsummary(list("Spatial Lag" = sp_lag_LDP_2012, -->
<!--                   "Spatial Error" = sp_err_LDP_2012,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_2012, -->
<!--                   "Spatial Lag" = sp_lag_LDP_rel_2012, -->
<!--                   "Spatial Error" = sp_err_LDP_rel_2012,  -->
<!--                   "Lag + Error" = sp_sacsar_LDP_rel_2012), -->
<!--              stars = TRUE, -->
<!--              add_rows = rows, -->
<!--              coef_map = cm -->
<!--              ) %>% -->
...
<!--   Turnout + LATperCapita + factor(Kaso.fusion) -->
...
<!-- tbl_sp_panel <- as.data.frame(cbind(sp_lag_turnout_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sp_er_turnout_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sarar_turnout_sum$CoefTable[c(3:14,2), c(1, 2, 4)])) -->
...
<!-- tbl_sp_panel <- as.data.frame(cbind(sp_lag_turnout_rel_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sp_er_turnout_rel_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sarar_turnout_rel_sum$CoefTable[c(3:14,2), c(1, 2, 4)])) -->
...
<!--   pack_rows("Fiscal type (base group = ‘Normal’)", 15, 23) %>% -->
<!--   row_spec(25, background = 'lightblue') %>% -->
<!--   row_spec(27, background = 'lightblue') %>% -->
...
<!-- tbl_sp_panel_LDP <- as.data.frame(cbind(sp_lag_LDP_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sp_er_LDP_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sarar_LDP_sum$CoefTable[c(3:14,2), c(1, 2, 4)])) -->
...
<!-- tbl_sp_panel_LDP <- as.data.frame(cbind(sp_lag_LDP_rel_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sp_er_LDP_rel_sum$CoefTable[c(2:13,1), c(1, 2, 4)], -->
<!--                                     sarar_LDP_rel_sum$CoefTable[c(3:14,2), c(1, 2, 4)])) -->
...
<iframe src="https://shohoku.share-wis.com/courses/15316/lectures/258358" data-external= "1" > </iframe>
