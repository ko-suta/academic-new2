...
addnbs2 <- function(sp.sample){
    ...
    queen_nb <- poly2nb(d7, row.names=d7$Code, queen=TRUE)
...
fm2 <- Turnout_relative ~ factor(Year) + Fstrength + EldPop + 
  LDP_relative + factor(Kaso.fusion)

fm3 <- LDP_relative ~ as.factor(Year) + Fstrength + EldPop + 
  Turnout_relative + Kaso.fusion
```


```{r include=FALSE}
spatial.4 <- spml(fm2, data = ElectionsSMD6, index = c("Code", "Year"), listw = nb_B3,
                  lag = TRUE, spatial.error = "none", model = "within", 
                  effect = "individual", 
                  na.action = na.exclude(), zero.policy = TRUE, legacy = FALSE)

sp_lag_sum <- summary(spatial.4)


spatial.4.LDP <- spml(fm3, data = ElectionsSMD6, index = c("Code", "Year"), listw = nb_B3,
                  lag = TRUE, spatial.error = "none", model = "within", 
                  effect = "individual", 
                  na.action = na.exclude(), zero.policy = TRUE, legacy = FALSE)

sp_lag_sum_LDP <- summary(spatial.4.LDP)
```


```{r include=FALSE}
spatial.4.2 <- spml(fm2, data = ElectionsSMD6, index = c("Code", "Year"), 
                    listw = nb_B3,
                    lag = FALSE, 
                    spatial.error = "b", 
                    model = "within", 
...
er.mod <- summary(spatial.4.2)


spatial.4.2.LDP <- spml(fm3, data = ElectionsSMD6, index = c("Code", "Year"), 
                    listw = nb_B3,
                    lag = FALSE, 
                    spatial.error = "b", 
                    model = "within", 
...
er.mod.LDP <- summary(spatial.4.2.LDP)
```


```{r include=FALSE}
spatial.4.1 <- spml(fm2, data = ElectionsSMD6, index = c("Code", "Year"), 
                    listw = nb_B3,
                    lag = TRUE, 
                    spatial.error = "b", 
                    model = "within", 
...
sar.mod <- summary(spatial.4.1)


spatial.4.1.LDP <- spml(fm3, data = ElectionsSMD6, index = c("Code", "Year"), 
                    listw = nb_B3,
                    lag = TRUE, 
                    spatial.error = "b", 
                    model = "within", 
...
sar.mod.LDP <- summary(spatial.4.1.LDP)
```



## Turnout ratio (relative)

```{r turnout-spatial, results='asis', echo=FALSE}
tbl_sp_panel <- as.data.frame(cbind(sp_lag_sum$CoefTable[c(2:11,1), c(1, 2, 4)],
                                    er.mod$CoefTable[c(2:11,1), c(1, 2, 4)],
                                    sar.mod$CoefTable[c(3:12,2), c(1, 2, 4)]))

tbl_sp_panel <- rbind(tbl_sp_panel,
                      c(rep(NA,6), sar.mod$CoefTable[1, c(1, 2, 4)]))
...
ar <- as.data.frame(rbind(c("R2", (round(sp_lag_sum$rsqr,2)), 
                            (round(er.mod$rsqr,2)), 
                            (round(sar.mod$rsqr,2)))))
...
tbl_sp_panel_LDP <- as.data.frame(cbind(sp_lag_sum_LDP$CoefTable[c(2:11,1), c(1, 2, 4)],
                                    er.mod.LDP$CoefTable[c(2:11,1), c(1, 2, 4)],
                                    sar.mod.LDP$CoefTable[c(3:12,2), c(1, 2, 4)]))

tbl_sp_panel_LDP <- rbind(tbl_sp_panel_LDP,
                      c(rep(NA,6), sar.mod.LDP$CoefTable[1, c(1, 2, 4)]))
...
ar <- as.data.frame(rbind(c("R2", (round(sp_lag_sum_LDP$rsqr,2)), 
                            (round(er.mod.LDP$rsqr,2)), 
                            (round(sar.mod.LDP$rsqr,2)))))
