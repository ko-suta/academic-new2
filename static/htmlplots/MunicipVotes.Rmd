...
ElectionsSMD <- read_csv("Data/ElectionsSMD.csv")

ElectionsSMD <- ElectionsSMD[,-c(6,9)]

ElectionsSMD <- ElectionsSMD[,c(7,6,1:5,8:16)]

ElectionsSMD2 <- ElectionsSMD %>%
...
```{r echo=FALSE, include=FALSE}
ElectionsSMD2$Year <- as.character(ElectionsSMD2$Year)
ElectionsSMD2$Year <- as.factor(ElectionsSMD2$Year)

ElectionsSMD2$Code <- as.character(ElectionsSMD2$Code)
ElectionsSMD2$Code <- as.factor(ElectionsSMD2$Code)

ElectionsSMD2 <- ElectionsSMD2 %>%
  filter(!Code %in% NA) %>%
  mutate(Turnout = ifelse(Turnout >=1, NA, Turnout),
         EldPop = EldPop/100)

Elections3 <- ElectionsSMD2[!duplicated(ElectionsSMD2[,1:2]),]
```
...
```{r echo=FALSE, include=FALSE}
TurnoutMod1 <- lm(Turnout ~ Year + Fstrength, data = Elections3)

TurnoutMod2 <- lm(Turnout ~ Year + Fstrength + EldPop + LDP, data = Elections3)

TurnoutMod3 <- lm(Turnout ~ Year + Fstrength + EldPop + LDP + 
                   Kaso.fusion, data = Elections3)
...
```{r echo=FALSE, include=FALSE}
LDPMod1 <- lm(LDP ~ Year + Fstrength, data = Elections3)

LDPMod2 <- lm(LDP ~ Year + Fstrength + EldPop + Turnout, data = Elections3)

LDPMod3 <- lm(LDP ~ Year + Fstrength + EldPop + Turnout + 
                   Kaso.fusion, data = Elections3)
...
ElectionsSMD2 %>%
  group_by(Year) %>%
  summarise(Turnout = mean(Turnout, na.rm = TRUE))
```
